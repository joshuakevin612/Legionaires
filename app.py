...
# Design tokens: warm cream base (Krafters Link palette), bold black ink,
# one amber accent. Risk badges keep semantic red/amber/green — the one
# deliberate exception to the palette, because a screening tool that mutes
# "high risk" to on-brand amber is a usability problem, not a style win.
DARK_CSS = """
<style>
...
:root {
    ...
    --yellow: #f2a90c;
    --yellow-soft: rgba(242, 169, 12, 0.16);
    --yellow-dim: #d99a0a;
    --text: #14110c;
    --text-muted: #6b6350;
    --red: #c62828;
    ...
    --green: #2f7d3c;
    --green-soft: rgba(47, 125, 60, 0.14);
}
...
/* ---- buttons (black pill, mirrors an "OPEN LINK" capsule button) ---- */
div.stButton > button {
    width: 100%;
    border-radius: 999px;
    border: none;
...
div.stButton > button:active { transform: translateY(0); box-shadow: 2px 2px 0px 0px var(--yellow); }
...
/* ---- progress bar: black track, amber gradient fill with shimmer ---- */
...
def plot_waveform(waveform: np.ndarray):
    """Render an ECG waveform as a cream-card, amber-lined matplotlib plot."""
    fig, ax = plt.subplots(figsize=(5, 2.6))
    fig.patch.set_facecolor("#fdf6d8")
    ax.set_facecolor("#fdf6d8")
    ax.plot(waveform, color="#d99a0a", linewidth=1.4)
    ...
    ax.grid(alpha=0.18, color="#14110c")
    fig.tight_layout()
    return fig
